...
INSERT INTO Employees
 values(41,"Siddharth","Gupta",3,70000, "2024-01-01","siddharth.gupta@example.com"),
...
INSERT INTO Employees
values ( 43,'Rahul','Mehra',11,58000,'2024-01-10','rahul.mehra@example.com'),
(44,'sara','Kapoor',11,62000,'2024-01-11','sara.kapoor@example.com'),
(45,'Nitin','Shaha',11,60000,'2024-01-12','nitin.shah@example.com');
...
(12, 'Research', 'Hyderabad', 'Dr. Meera Josh', 2000000);
...
select * from employees where month(DateOfJoining) = 02 ;
...
select * from employees where salary >55000 and DateOfJoining<'2024-03-01' ;
...
select departmentid,count(*) as emp_count from employees group by departmentid ;
...
select departmentid,min(dateofjoining) as date_of_joining from employees group by departmentid ;
...
select departmentid,sum(salary) as totalsalary from employees where salary >6000  group by departmentid ;


-- Situation -8: To monitor departmental growth, count the number of employees who joined in 
-- 2024 in each department.
select departmentid,count(*) as emp_count from employees where year(DateOfJoining) = 2024 group by departmentid ;
...
select departmentid,count(*) as empcount from employees where salary between 50000 and 70000   group by departmentid ;


-- Situation -10: For diversity analysis, count the number of employees whose names start with 
-- each letter of the alphabet. 
select left(firstname,1) as startingletter,count(*) as countemp from employees
group by startingletter order by startingletter  ;


-- Department Table
-- Situation -11: For city-based analysis, count the number of departments located in each city. 
select location,count(departmentid) as departmentcount from departments group by location ;
...
select HeadOfDepartment,count(departmentid) as countofdepartment from departments group by HeadOfDepartment ;
...
select location,max(annualbudget) as max_budget,min(annualbudget) as min_budget from departments group by location ;
...
-- name and a comma.
select concat("Hello"," ",firstname) as greetings from employees ;

-- Situation -4: For creating a unique employee ID, concatenate the department ID and employee number.
select concat(employeeid," | ",DepartmentID) as emp_details from employees ;
...
update employees set salary =salary-salary*.1 where departmentid=3 ; 
...
update employees set job_title ="Senior Developer" where employeeid=101 ;
...
update  employees set phone=9876543210 where employeeid=120 ;
...
update departments set departmentname ='Human Resource' where departmentid = 5 ;
...
update employees set project='alpha' where employeeid=110 ; 
...
delete from employees where dateofjoining < 2020-01-01 ; 
...
delete from employees where project_name =  "Project Beta" ;
...
delete from employees where region = 'north' and retired_date < 2023-01-01 ;
use employeemanagement ;
...
alter table employees add column departmentname varchar(30) ;
...
-- and accessibility for employees prior to 2020.
 ...
 --  JoinDate Column to Employee Table
...
-- Populate JoinDate for Existing Employee Records
UPDATE employees
SET JoinDate = DateOfJoining
WHERE JoinDate IS NULL;

-- enable safe update mode
SET SQL_SAFE_UPDATES = 1;

-- Modify Primary Key to Include JoinDate
ALTER TABLE employees
DROP PRIMARY KEY,
ADD PRIMARY KEY (EmployeeID, JoinDate);

-- Partition Employee Table Based on JoinDate
ALTER TABLE employees
...
-- Retrieve Employees Who Joined Before 2020
SELECT *
FROM employees
...
-- Ensure the department table has a primary key  
